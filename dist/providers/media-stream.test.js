...
function createMockMediaStream(active = true) {
    const mockTrack = {
...
import { MediaStreamProvider, createMediaStreamProvider, MediaStreamEndedError, MediaStreamInactiveError, NoVideoTrackError, } from "./media-stream";
describe("MediaStreamProvider", () => {
    ...
    describe("createMediaStreamProvider factory", () => {
        ...
        it("throws MediaStreamInactiveError for inactive stream", async () => {
            ...
            await expect(createMediaStreamProvider(stream)).rejects.toThrow(MediaStreamInactiveError);
        });
        it("throws NoVideoTrackError when stream has no video tracks", async () => {
            ...
            await expect(createMediaStreamProvider(stream)).rejects.toThrow(NoVideoTrackError);
        });
    ...
    describe("frame extraction", () => {
        ...
        it("extracts image data from canvas", async () => {
            ...
            const provider = await createMediaStreamProvider(stream);
            await provider.getNextFrame();
            expect(mockCanvasContext.getImageData).toHaveBeenCalledWith(0, 0, 640, 480);
        });
    ...
    describe("frame rate limiting", () => {
        it("calculates correct frame interval from fps", async () => {
            ...
            expect(provider.getFrameInterval()).toBeCloseTo(expectedInterval);
        });
        ...
        it("applies frame rate limiting when fps is specified", async () => {
            ...
            expect(provider.getFrameInterval()).toBe(100);
        });
    });
    ...
    describe("edge cases", () => {
        it("throws Error when canvas.getContext returns null", async () => {
            ...
            const stream = createMockMediaStream();
            await expect(createMediaStreamProvider(stream)).rejects.toThrow("Failed to get 2D rendering context");
...
//# sourceMappingURL=media-stream.test.js.map