...
export class PhaseDetector {
    ...
    analyzeFrames(sequence, startFrame, endFrame) {
        ...
        // Apply smoothing
        const smoothedWristY = this.config.smoothingWindowSize > 1
            ? movingAverage(wristYValues, this.config.smoothingWindowSize)
            : wristYValues;
        const smoothedWristX = this.config.smoothingWindowSize > 1
            ? movingAverage(wristXValues, this.config.smoothingWindowSize)
            : wristXValues;
        const smoothedHipY = this.config.smoothingWindowSize > 1
        ...
        for (let i = startFrame; i <= endFrame; i++) {
            ...
            const leftKneeAngle = this.calculateKneeAngle(leftHip, leftKnee, leftAnkle);
            const rightKneeAngle = this.calculateKneeAngle(rightHip, rightKnee, rightAnkle);
            ...
            const avgConfidence = (leftWrist.confidence +
            ...
            const earX = leftEar.confidence + rightEar.confidence > 0
    ...
    calculateKneeAngle(hip, knee, ankle) {
        return calculateAngle({ x: hip.x, y: hip.y, z: hip.z }, { x: knee.x, y: knee.y, z: knee.z }, { x: ankle.x, y: ankle.y, z: ankle.z });
    }
    /**
    ...
    findSetPointFrame(frameData, state, riseStartFrame, baseFrame) {
        ...
        const startIdx = Math.max(0, (riseStartFrame >= 0 ? riseStartFrame : baseFrame) - baseFrame);
        // Need a few frames of rise to find a turning point within.
        if (peakIdx - startIdx < 2)
            return null;
    ...
    assignPhases(frameData, state, baseFrame) {
        ...
        const turningFrame = this.findSetPointFrame(frameData, state, riseStart, baseFrame);
        ...
        if (setPointEnd !== -1) {
            ...
            if (remainingFrames <= typicalReleaseDuration) {
                ...
                separationEnd = Math.max(separationStart, baseFrame + n - 1 - Math.min(2, Math.floor(remainingFrames / 2)));
            }
            else {
        ...
        if (afterRelease !== -1 && afterRelease <= lastFrameIndex) {
            ...
            if (followThroughStart <= followThroughEnd &&
    ...
    calculateOverallConfidence(frameData, phases) {
        ...
        const avgLandmarkConfidence = frameData.reduce((sum, f) => sum + f.avgConfidence, 0) / frameData.length;
 ...
 */
export function createPhaseDetector(config = {}) {
