...
function createFrameSequence(configs) {
    return configs.map((config) => {
        ...
        landmarks[LANDMARK_INDEX.LEFT_WRIST] = createLandmark(leftWristX, config.wristY[0]);
        landmarks[LANDMARK_INDEX.RIGHT_WRIST] = createLandmark(rightWristX, config.wristY[1]);
...
describe("PhaseDetector", () => {
    ...
    describe("set point detection", () => {
        ...
        it("side-on shot: set point is the horizontal turning point, before the wrist-height peak", () => {
            ...
            if (rise)
                expect(rise.endFrame).toBeLessThan(setPoint.startFrame);
            if (release)
