...
function calculateElbowFlareFromPlane(shoulder, elbow, bodyNormal) {
    ...
    const magnitude = Math.sqrt(shoulderToElbow.x * shoulderToElbow.x +
    ...
    const dotProduct = shoulderToElbow.x * bodyNormal.x +
 ...
 */
export class ShootingElbowFlareCalculator {
    name = "shootingElbowFlare";
    description = "Angle of shooting elbow relative to body plane at release (degrees)";
    unit = "degrees";
    calculate(context) {
        ...
        const bodyNormal = calculateBodyPlaneNormal(leftShoulder.position, rightShoulder.position, leftHip.position, rightHip.position);
        // Calculate elbow flare angle
        const flareAngle = calculateElbowFlareFromPlane(shootingShoulder.position, shootingElbow.position, bodyNormal);
        const value = {
 ...
 */
export class ShootingElbowAngleCalculator {
    name = "shootingElbowAngle";
    description = "Bend angle at elbow (shoulder-elbow-wrist) at set point (degrees)";
    unit = "degrees";
    calculate(context) {
        const { poseLandmarks, phases, config } = context;
        const mapping = getHandednessMapping(config.shootingHand);
        ...
        const elbowAngle = calculateAngle(shoulder.position, elbow.position, wrist.position);
 ...
 */
export class MaxArmExtensionCalculator {
    name = "maxArmExtension";
    description = "Maximum elbow extension achieved during follow-through (degrees)";
    unit = "degrees";
    calculate(context) {
        const { poseLandmarks, phases, config } = context;
        const mapping = getHandednessMapping(config.shootingHand);
 ...
 */
export class WristSnapAngleCalculator {
    name = "wristSnapAngle";
    description = "Wrist flexion change from set point to release (degrees)";
    unit = "degrees";
    calculate(context) {
        const { poseLandmarks, phases, config } = context;
        const mapping = getHandednessMapping(config.shootingHand);
        ...
        const setPointWristAngle = calculateAngle(setPointElbow.position, setPointWrist.position, setPointFinger.position);
        // Calculate wrist angle at release
        const releaseWristAngle = calculateAngle(releaseElbow.position, releaseWrist.position, releaseFinger.position);
 ...
 */
export class FollowThroughHoldCalculator {
    name = "followThroughHold";
    description = "Duration arm stays extended during follow-through (% of shot)";
    unit = "percent";
    calculate(context) {
 ...
 */
export function createShootingArmCalculators() {
    return [
...
//# sourceMappingURL=shooting-arm.js.map