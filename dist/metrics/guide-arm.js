 ...
 */
export class GuideElbowFlareCalculator {
    name = "guideElbowFlare";
    description = "Angle of guide elbow relative to body plane at set point (degrees)";
    unit = "degrees";
    calculate(context) {
        ...
        const flareAngle = calculateElbowFlareFromPlane(guideShoulder.position, guideElbow.position, bodyNormal);
        const value = {
 ...
 */
export class GuideHandPositionCalculator {
    name = "guideHandPosition";
    description = "Position of guide hand relative to ball/shooting hand at set point";
    unit = "category";
    calculate(context) {
        const { poseLandmarks, phases, config } = context;
        const mapping = getHandednessMapping(config.shootingHand);
        ...
        const guideIndexIdx = config.shootingHand === "right"
        ...
        if (guideIndex)
            landmarks.push(guideIndex);
        const confidence = calculateMinConfidence(landmarks);
        // Classify position based on relative positions
        const category = this.classifyPosition(guideWrist.position, shootingWrist.position, guideIndex?.position, config.shootingHand);
        const value = {
            value: category,
            unit: this.unit,
            frame: setPointPhase.startFrame,
 ...
 */
export class GuideHandReleaseCalculator {
    name = "guideHandRelease";
    description = "When guide hand releases from ball (% of shot duration)";
    unit = "percent";
    calculate(context) {
        ...
        const guideIndexIdx = config.shootingHand === "right"
            ? LANDMARK_INDICES.LEFT_INDEX
            : LANDMARK_INDICES.RIGHT_INDEX;
        const shootingIndexIdx = config.shootingHand === "right"
            ? LANDMARK_INDICES.RIGHT_INDEX
            : LANDMARK_INDICES.LEFT_INDEX;
        ...
        for (let frameIndex = frameRange.start; frameIndex <= frameRange.end; frameIndex++) {
            ...
            if (!guideIndex || !shootingIndex)
                continue;
            const distance = calculateDistance(guideIndex.position, shootingIndex.position);
 ...
 */
export function createGuideArmCalculators() {
    return [
...
//# sourceMappingURL=guide-arm.js.map