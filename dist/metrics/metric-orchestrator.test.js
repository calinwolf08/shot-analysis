 ...
 */
function createMockPoseLandmarks(frameIndex, confidence = 0.9) {
    const landmarks = Array.from({ length: 33 }, () => createMockLandmark());
    return {
        landmarks,
        confidence,
 ...
 */
function createMockCalculator(name, returnValue) {
    return {
        name,
...
describe("MetricOrchestrator", () => {
    ...
    describe("registerCalculator", () => {
        ...
        it("overwrites calculator with same name", () => {
            ...
            expect(orchestrator.getCalculatorNames().filter((name) => name === "elbowAngle")).toHaveLength(1);
        });
    });
    ...
    describe("calculateMetrics", () => {
        ...
        it("returns empty metrics when no calculators registered", () => {
            ...
            const result = orchestrator.calculateMetrics(poseLandmarks, { start: 0, end: 0 }, {}, config);
        ...
        it("calculates metrics from all registered calculators", () => {
            ...
            const result = orchestrator.calculateMetrics(poseLandmarks, { start: 0, end: 1 }, {}, config);
        ...
        it("handles calculator that returns both value and error", () => {
            ...
            const orchestrator = new MetricOrchestrator([calculator]);
            const result = orchestrator.calculateMetrics([createMockPoseLandmarks(0)], { start: 0, end: 0 }, {}, config);
    ...
    describe("analyzeShot", () => {
        ...
        it("returns ShotAnalysis with calculated metrics", () => {
            ...
            const orchestrator = new MetricOrchestrator([calculator]);
            const poseLandmarks = Array.from({ length: 71 }, (_, i) => createMockPoseLandmarks(10 + i));
            const phases = {
                [ShotPhase.Gather]: { startFrame: 10, endFrame: 20 },
                [ShotPhase.Release]: { startFrame: 60, endFrame: 65 },
            };
            const analysis = orchestrator.analyzeShot(0, poseLandmarks, { start: 10, end: 80 }, phases, config);
        ...
        it("calculates overall confidence as average of metric confidences", () => {
            ...
            const orchestrator = new MetricOrchestrator([calc1, calc2]);
            const analysis = orchestrator.analyzeShot(0, [createMockPoseLandmarks(0)], { start: 0, end: 0 }, {}, config);
            // (0.8 + 1.0) / 2 = 0.9
        ...
        it("correctly sets shot index", () => {
            const orchestrator = new MetricOrchestrator();
            const analysis1 = orchestrator.analyzeShot(0, [createMockPoseLandmarks(0)], { start: 0, end: 50 }, {}, config);
            const analysis2 = orchestrator.analyzeShot(5, [createMockPoseLandmarks(100)], { start: 100, end: 150 }, {}, config);
...
//# sourceMappingURL=metric-orchestrator.test.js.map