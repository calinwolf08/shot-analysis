 ...
 */
export function areHandsTogether(pose, threshold = DEFAULT_HAND_TOGETHER_THRESHOLD) {
 ...
 */
export function inferBallCenter(pose) {
    const leftIndex = pose.landmarks[LANDMARK_INDICES.LEFT_INDEX];
    const rightIndex = pose.landmarks[LANDMARK_INDICES.RIGHT_INDEX];
    if (!leftIndex || !rightIndex) {
        return null;
 ...
 */
export class BallDipCalculator {
    name = "ballDip";
    description = "How far ball drops before rising to set point (normalized to shoulder width)";
    unit = "normalized";
    calculate(context) {
 ...
 */
export class BallPathCalculator {
    name = "ballPath";
    description = "Straightness of ball path to set point (deviation score, 0 = straight)";
    unit = "deviation";
    calculate(context) {
        const { poseLandmarks, phases, frameRange } = context;
 ...
 */
export class SetPointHeightCalculator {
    name = "setPointHeight";
    description = "Height of ball at set point relative to head (normalized to shoulder width)";
    unit = "normalized";
    calculate(context) {
 ...
 */
export class SetPointDurationCalculator {
    name = "setPointDuration";
    description = "Duration ball stays at set point (milliseconds)";
    unit = "ms";
    calculate(context) {
        ...
        if (startPose && endPose && endPose.timestamp > startPose.timestamp) {
            ...
            const fps = 1000 /
                Math.max(1, (poseLandmarks[1]?.timestamp ?? 33.33) -
 ...
 */
export class ReleasePointCalculator {
    name = "releasePoint";
    description = "Ball height at release point relative to head (normalized to shoulder width)";
    unit = "normalized";
    calculate(context) {
        const { poseLandmarks, phases, config } = context;
        // Get release phase
 ...
 */
export class ReleaseAngleCalculator {
    name = "releaseAngle";
    description = "Angle of shooting arm at release point (degrees from horizontal)";
    unit = "degrees";
    calculate(context) {
        const { poseLandmarks, phases, config } = context;
        const mapping = getHandednessMapping(config.shootingHand);
 ...
 */
export class BallBehindHeadCalculator {
    name = "ballBehindHead";
    description = "Furthest back position of ball relative to head (normalized to shoulder width)";
    unit = "normalized";
    calculate(context) {
 ...
 */
export function createBallMetricCalculators() {
    return [
