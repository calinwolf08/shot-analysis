 ...
 */
function createSetPointReleaseSequence(startFrame, frameCount, setPointFrame, releaseFrame) {
    const frames = [];
    for (let i = 0; i < frameCount; i++) {
        const frameIdx = startFrame + i;
        const landmarks = createDefaultLandmarks();
        ...
        landmarks[LANDMARK_INDICES.LEFT_WRIST] = createLandmark(0.4 + wristXOffset, wristY, 0);
        landmarks[LANDMARK_INDICES.RIGHT_WRIST] = createLandmark(0.6 + wristXOffset, wristY, 0);
        ...
        if (i < releaseFrame) {
            ...
            const progress = Math.max(0, (i - setPointFrame) / (releaseFrame - setPointFrame));
        ...
        landmarks[LANDMARK_INDICES.RIGHT_INDEX] = createLandmark(indexX + 0.2, indexY, 0);
        frames.push(createFrame(frameIdx, landmarks));
    }
    return frames;
}
...
describe("detectRelease", () => {
    ...
    it("skips frames with low visibility landmarks", () => {
        ...
        const landmarks = [...frames[12].landmarks];
...
describe("KeyframeDetector.detectSetPointReleaseKeyframes", () => {
    ...
    it("uses custom setPointMaxElbowAngle", () => {
        ...
        const setPoint1 = result1.keyframes.find((k) => k.keyframeId === "set_point");
        const setPoint2 = result2.keyframes.find((k) => k.keyframeId === "set_point");
