 ...
 */
function createTestProfile(name, targets = {}) {
    return {
        name,
 ...
 */
function createNumericTarget(ideal, min, max) {
    return {
        ideal,
        acceptable: { min, max },
        priority: "medium",
...
describe("ProfileRegistry", () => {
    describe("constructor and built-in profiles", () => {
        ...
        it("returns correct built-in profile objects", () => {
            const registry = new ProfileRegistry();
            expect(registry.get("youth-fundamentals")).toEqual(youthFundamentalsProfile);
    ...
    describe("registerProfile() with invalid profiles", () => {
        ...
        it("throws error with descriptive message for empty name", () => {
            const invalidProfile = createTestProfile("");
            expect(() => registry.register(invalidProfile)).toThrow(/name.*empty|empty.*name/i);
        });
    ...
    describe("registerProfile() override behavior", () => {
        ...
        it("allows overriding built-in profile with custom profile", () => {
            ...
            expect(registry.get("youth-fundamentals")?.description).toBe("Custom youth profile");
        });
        it("logs warning when overriding existing profile", () => {
            const customProfile = createTestProfile("youth-fundamentals");
            registry.register(customProfile);
            expect(consoleSpy).toHaveBeenCalled();
            expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/overrid|replac|exist/i));
        });
    ...
    describe("getProfile()", () => {
        ...
        it("throws descriptive error for non-existent profile", () => {
            expect(() => registry.get("unknown-profile")).toThrow(/unknown-profile.*not found|not found.*unknown-profile|does not exist/i);
        });
        it("suggests available profiles in error message", () => {
            expect(() => registry.get("missing")).toThrow(/available|youth-fundamentals|high-school|pro-form/i);
        });
