 ...
 */
function createMetricValue(value, confidence = 0.95) {
    return {
        value,
 ...
 */
function createNumericTarget(ideal, min, max, priority = "high") {
    return {
        ideal,
        acceptable: { min, max },
        priority,
 ...
 */
function createCategoricalTarget(ideal, acceptable, priority = "medium") {
    return {
        ideal,
        acceptable,
        priority,
 ...
 */
function createTestProfile(targets) {
    return {
        name: "test-profile",
 ...
 */
function createTestShotAnalysis(metrics) {
    return {
        shotIndex: 0,
...
describe("ProfileComparisonEngine", () => {
    ...
    describe("categorical metric comparison", () => {
        ...
        it("returns fail status when value is not in acceptable values", () => {
            ...
            expect(result.metrics.handPosition?.feedback).toBe("Value not in acceptable options: cup, hinge");
        });
    ...
    describe("feedback message selection", () => {
        ...
        it("selects incorrect feedback for categorical mismatch", () => {
            ...
            const result = engine.compareToProfile(shot, profile);
            expect(result.metrics.handPosition?.feedback).toBe("Hand position should be cup or hinge");
        });
        it("uses default feedback when profile feedback is not specified", () => {
            ...
            const result = engine.compareToProfile(shot, profile);
            expect(result.metrics.angle?.feedback).toBe("Value is below the acceptable range");
        });
    ...
    describe("summary generation", () => {
        ...
        it("includes feedback text in priority issues", () => {
            ...
            expect(result.summary.priorityIssues[0]).toContain("Elbow angle too closed");
        });
