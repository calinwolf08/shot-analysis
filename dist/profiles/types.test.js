...
import { formProfileSchema, metricTargetSchema, metricFeedbackSchema, numericRangeSchema, profileComparisonSchema, comparisonSummarySchema, metricComparisonResultSchema, validateProfile, safeValidateProfile, } from "./schemas";
import { createEmptyComparisonSummary, createEmptyProfileComparison, isNumericTarget, isCategoricalTarget, getFeedbackMessage, DEFAULT_FEEDBACK_MESSAGES, } from "./types";
...
describe("formProfileSchema", () => {
    ...
    describe("invalid profiles", () => {
        it("rejects empty profile name", () => {
            ...
            expect(result.success).toBe(false);
            if (!result.success) {
                expect(result.error.issues.some((issue) => issue.message.includes("empty"))).toBe(true);
            }
        });
...
describe("type helper functions", () => {
    ...
    describe("getFeedbackMessage", () => {
        ...
        it("returns tooLow message for numeric target below range", () => {
            ...
            expect(getFeedbackMessage(target, "fail", true, true)).toBe("Elbow angle too low");
        });
        it("returns tooHigh message for numeric target above range", () => {
            ...
            expect(getFeedbackMessage(target, "fail", true, false)).toBe("Elbow angle too high");
        });
        it("returns incorrect message for categorical target", () => {
            ...
            expect(getFeedbackMessage(target, "fail", false)).toBe("Invalid hand position");
        });
        it("returns default tooLow message when not specified", () => {
            ...
            expect(getFeedbackMessage(target, "fail", true, true)).toBe(DEFAULT_FEEDBACK_MESSAGES.tooLow);
        });
        it("returns default tooHigh message when not specified", () => {
            ...
            expect(getFeedbackMessage(target, "fail", true, false)).toBe(DEFAULT_FEEDBACK_MESSAGES.tooHigh);
        });
        it("returns default incorrect message when not specified", () => {
            ...
            expect(getFeedbackMessage(target, "fail", false)).toBe(DEFAULT_FEEDBACK_MESSAGES.incorrect);
        });
    });
