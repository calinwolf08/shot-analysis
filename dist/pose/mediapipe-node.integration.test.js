...
import { createMediaPipeNodeDetector, DEFAULT_MODEL_PATH, MODEL_PATHS, ModelNotFoundError, WasmInitializationError, ModelCreationError, } from "./mediapipe-node";
 ...
 */
async function checkMediaPipeAvailable() {
    try {
 ...
 */
function createColorTestImage(width, height) {
 ...
 */
function createTestFrame(width = 640, height = 480, data) {
    return {
...
describe("MediaPipeNodeDetector integration tests", () => {
    beforeAll(async () => {
        ...
        if (!mediapipeAvailable) {
            console.log("MediaPipe runtime not available in this environment, skipping integration tests");
        }
    });
    describe("initialization", () => {
        ...
        it.skipIf(!mediapipeAvailable)("initializes with default model from CDN", async () => {
            detector = await createMediaPipeNodeDetector();
            expect(detector).toBeDefined();
        });
        it.skipIf(!mediapipeAvailable)("initializes with custom confidence settings", async () => {
    ...
    describe("pose detection", () => {
        ...
        it.skipIf(!mediapipeAvailable)("returns null when no person in frame", async () => {
        ...
        it.skipIf(!mediapipeAvailable)("handles empty image gracefully", async () => {
        ...
        it.skipIf(!mediapipeAvailable)("handles solid white image gracefully", async () => {
        ...
        it.skipIf(!mediapipeAvailable)("handles multiple sequential detections", async () => {
            const frame = createTestFrame(640, 480);
    ...
    describe("resource cleanup", () => {
        it.skipIf(!mediapipeAvailable)("close() releases resources without errors", async () => {
            const testDetector = await createMediaPipeNodeDetector();
            await expect(testDetector.close()).resolves.not.toThrow();
        });
        it.skipIf(!mediapipeAvailable)("throws DetectorClosedError when used after close", async () => {
            const testDetector = await createMediaPipeNodeDetector();
            const frame = createTestFrame();
            await testDetector.close();
            await expect(testDetector.detect(frame)).rejects.toThrow(DetectorClosedError);
        });
        it.skipIf(!mediapipeAvailable)("throws DetectorClosedError when closed twice", async () => {
            const testDetector = await createMediaPipeNodeDetector();
            await testDetector.close();
    ...
    describe("error handling", () => {
        ...
        it.skipIf(!mediapipeAvailable)("throws ModelNotFoundError for invalid model path", async () => {
            await expect(createMediaPipeNodeDetector({ modelPath: "/nonexistent/model.task" })).rejects.toThrow(ModelNotFoundError);
        });
        it.skipIf(!mediapipeAvailable)("ModelNotFoundError contains the model path", async () => {
            try {
                ...
                expect(error.modelPath).toBe("/nonexistent/custom-model.task");
            }
        });
    });
    describe("result format validation", () => {
        ...
        it.skipIf(!mediapipeAvailable)("returned PoseLandmarks has correct structure", async () => {
    ...
    describe("constants", () => {
        it("exports DEFAULT_MODEL_PATH pointing to MediaPipe CDN", () => {
            expect(DEFAULT_MODEL_PATH).toContain("storage.googleapis.com/mediapipe-models");
...
//# sourceMappingURL=mediapipe-node.integration.test.js.map