...
import { ModelNotFoundError, WasmInitializationError, ModelCreationError, createMediaPipeNodeDetector, DEFAULT_MODEL_PATH, MODEL_PATHS, } from "./mediapipe-node";
...
describe("MediaPipeNodeDetector", () => {
    ...
    describe("initialization errors", () => {
        ...
        it("throws ModelNotFoundError when model file not found (ENOENT)", async () => {
            vi.mocked(PoseLandmarker.createFromOptions).mockRejectedValue(new Error("ENOENT: no such file or directory"));
            await expect(createMediaPipeNodeDetector({ modelPath: "/nonexistent/model.task" })).rejects.toThrow(ModelNotFoundError);
        });
        ...
        it("throws WasmInitializationError for WASM loading failures", async () => {
            ...
            await expect(createMediaPipeNodeDetector()).rejects.toThrow(WasmInitializationError);
        });
        ...
        it("throws ModelCreationError for other model creation failures", async () => {
            ...
            await expect(createMediaPipeNodeDetector()).rejects.toThrow(ModelCreationError);
        });
        it("ModelCreationError contains original error", async () => {
            const originalError = new Error("Invalid configuration");
            vi.mocked(PoseLandmarker.createFromOptions).mockRejectedValue(originalError);
            try {
                await createMediaPipeNodeDetector();
...
//# sourceMappingURL=mediapipe-node.test.js.map