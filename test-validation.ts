...
const VIDEO_PATH = process.argv[2] || '/mnt/c/Users/calin/OneDrive - Soul Focused Group/Personal Photos/basketball clips/chris/chris 5.mp4';
...
async function runTest() {
  ...
  try {
    ...
    await page.goto(`http://localhost:${SERVER_PORT}`, { waitUntil: 'networkidle' });
    ...
    try {
      await page.waitForFunction(() => {
        ...
        const metricsVisible = metricsContent && !metricsContent.classList.contains('hidden');
      ...
      console.log('Timeout waiting for analysis - capturing current state anyway');
    }
    ...
    const result = await page.evaluate(() => {
      ...
      const noDataVisible = !document.getElementById('noDataMessage')?.classList.contains('hidden');
