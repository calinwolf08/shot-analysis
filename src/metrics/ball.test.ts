 ...
 */
function createHandsTogetherPose(
  frameIndex: number,
  options: {
    // Ball position (via index fingers)
    ballX?: number;
    ballY?: number;
    ballZ?: number;
    // Separation between index fingers
    fingerSeparation?: number;
    // Nose (head) position for height reference
    noseY?: number;
    ...
    shoulderY?: number;
    // Visibility
    leftIndexVisibility?: number;
    rightIndexVisibility?: number;
  } = {},
): PoseLandmarks {
  const {
    ballX = 0.5,
    ballY = 0.3,
    ballZ = 0,
    fingerSeparation = 0.02, // Close together by default
    noseY = 0.2,
    leftShoulderX = 0.4,
    rightShoulderX = 0.6,
    shoulderY = 0.35,
    leftIndexVisibility = 1.0,
    rightIndexVisibility = 1.0,
  } = options;
 ...
 */
function createHandsSeparatedPose(
  frameIndex: number,
  options: {
    leftIndexX?: number;
    rightIndexX?: number;
...
describe("SetPointDurationCalculator", () => {
  ...
  describe("basic functionality", () => {
    ...
    it("handles very short set point (quick release)", () => {
      ...
      const poseLandmarks = Array.from({ length: 3 }, (_, i) =>
        createHandsTogetherPose(i),
      );
