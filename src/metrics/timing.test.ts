...
import type { MetricCalculatorContext, MetricCalculator } from "./types";
 ...
 */
function createPoseWithWristHeight(
  frameIndex: number,
  wristY: number,
  kneeAngleNormalized: number = 0.5, // 0 = bent, 1 = extended
  timestamp: number | null = null,
): PoseLandmarks {
...
describe("BallLegSyncCalculator", () => {
  describe("basic functionality", () => {
    ...
    it("returns ~0 when ball and legs are perfectly synchronized", () => {
      ...
      expect(Math.abs(result.value?.value as number)).toBeLessThanOrEqual(15);
    });
  });
...
describe("TotalShotDurationCalculator", () => {
  describe("basic functionality", () => {
    ...
    it("calculates total duration from gather to follow-through in milliseconds", () => {
      ...
      const poseLandmarks = Array.from({ length: 10 }, (_, i) =>
        createPoseWithWristHeight(i, 0.5 - i * 0.05, 0.5, i * 33.33),
      );
  ...
  describe("edge cases", () => {
    it("handles very quick shot (<500ms)", () => {
      ...
      const poseLandmarks = Array.from({ length: 10 }, (_, i) =>
        createPoseWithWristHeight(i, 0.5 - i * 0.05, 0.5, i * 16.67),
      );
    ...
    it("handles hesitation/pump fake with extended duration", () => {
      ...
      const poseLandmarks = Array.from({ length: 30 }, (_, i) =>
        createPoseWithWristHeight(i, 0.5 - i * 0.01, 0.5, i * 33.33),
      );
...
describe("createTimingCalculators", () => {
  it("returns all timing calculators", () => {
    ...
    const names = calculators.map((c: MetricCalculator) => c.name);
