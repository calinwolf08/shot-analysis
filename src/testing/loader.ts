 ...
 */
export function loadPoseData(filePath: string): LoadPoseDataResult {
 ...
 */
export function loadLabelData(filePath: string): LoadLabelDataResult {
  // Check if file exists
  if (!fs.existsSync(filePath)) {
    return { success: false, error: `File not found: ${filePath}` };
  }
...
export function discoverTestCases(
  options: DiscoverTestCasesOptions = {},
): TestCaseDiscoveryResult {
  const { testDataDir = DEFAULT_TEST_DATA_DIR, createIfMissing = false } =
    options;
 ...
 */
export function reportDiscoveryResults(result: TestCaseDiscoveryResult): void {
  console.log("\n=== Test Case Discovery ===\n");
  ...
  if (result.testCases.length > 0) {
    ...
    for (const testCase of result.testCases) {
      ...
      console.log(
        `  - ${testCase.name}: ${shotCount} shot(s), ${frameCount} frame(s)`,
      );
    }
  } else {
