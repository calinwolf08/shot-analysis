 ...
 */
export function calculateStats(data: TestResults): CalculatedStats {
	// Initialize stats for each keyframe
	const keyframeStats: Record<string, KeyframeStats> = {};
 ...
 */
export function calculateAvgError(stats: KeyframeStats): number | null {
	if (stats.diffs.length === 0) return null;
 ...
 */
export function calculateMaxError(stats: KeyframeStats): number | null {
	if (stats.diffs.length === 0) return null;
 ...
 */
export function calculatePassRate(stats: KeyframeStats): number | null {
 ...
 */
export function printStats(calculatedStats: CalculatedStats): void {
	const { keyframeStats, overallStats } = calculatedStats;

	console.log(
		'# Keyframe Detection Statistics (8 Videos, ' +
	...
	console.log(
		'- **Total Labeled Keyframes**: ' + overallStats.totalLabeledKeyframes
	);
	console.log(
		'- **Keyframes Passing**: ' +
...
if (isMain) {
	const data: TestResults = JSON.parse(
		fs.readFileSync('test-data/test-results.json', 'utf8')
	);
