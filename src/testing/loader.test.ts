...
import type { TestCaseDiscoveryResult } from "./types";
...
type MockDirents = any;

/** Helper to create a mock Dirent entry */
function createMockDirent(name: string, isDir: boolean): MockDirents {
  return {
    name,
    isDirectory: () => isDir,
    isFile: () => !isDir,
...
describe("loadPoseData", () => {
  ...
  it("warns for empty frames array", () => {
    ...
    mockFs.existsSync.mockReturnValue(true);
    mockFs.readFileSync.mockReturnValue(
      JSON.stringify(poseDataWithEmptyFrames),
    );
    ...
    expect(consoleWarnSpy).toHaveBeenCalledWith(
      expect.stringContaining("empty frames array"),
    );
...
describe("discoverTestCases", () => {
  ...
  it("returns empty result when no directories exist", () => {
    ...
    expect(result.testCases).toHaveLength(0);
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining("No test case directories"),
    );
  ...
  it("skips directories missing poses.json", () => {
    mockFs.existsSync.mockImplementation((p) => {
      ...
      if (pathStr.endsWith("test-data")) return true;
      if (pathStr.includes("poses.json")) return false;
      if (pathStr.includes("labels.json")) return true;
      return true;
    });
  ...
  it("skips directories missing labels.json", () => {
    mockFs.existsSync.mockImplementation((p) => {
      const pathStr = String(p);
      if (pathStr.endsWith("test-data")) return true;
      if (pathStr.includes("poses.json")) return true;
      if (pathStr.includes("labels.json")) return false;
      return true;
    });
...
describe("reportDiscoveryResults", () => {
  it("reports discovered test cases", () => {
    ...
    const result: TestCaseDiscoveryResult = {
      testCases: [
        {
          name: "test-1",
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining("1 valid test case"),
    );
    expect(consoleLogSpy).toHaveBeenCalledWith(
  ...
  it("reports skipped directories", () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining("missing poses.json"),
    );
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining("missing labels.json"),
    );
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining("missing poses.json and labels.json"),
    );
  ...
  it("reports errors", () => {
    ...
    expect(consoleLogSpy).toHaveBeenCalledWith(
      expect.stringContaining("error-1/poses.json"),
    );
