...
function createFailingResult(
  video: string = "failing-video.mp4",
): ComparisonResult {
  return {
    video,
    status: "fail",
 ...
 */
function createNoShotsResult(video: string = "no-shots.mp4"): ComparisonResult {
  return {
    video,
    status: "fail",
    orientation: {
...
describe("saveJsonReport", () => {
  ...
  it("writes pretty-printed JSON", () => {
    ...
    const parsed = JSON.parse(content) as TestReport;
  ...
  it("returns error on permission issues", () => {
    ...
    const results = [createPassingResult()];
    const result = saveJsonReport(
      results,
      path.join(testDir, "subdir", "test.json"),
    );
