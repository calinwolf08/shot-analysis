...
describe("analysisConfigSchema", () => {
  describe("valid configurations", () => {
    ...
    it("accepts all valid timing units", () => {
      const units: Array<"frames" | "ms" | "percent"> = [
        "frames",
        "ms",
        "percent",
      ];
    ...
    it("accepts minConfidenceThreshold of 0", () => {
      const config = {
        ...
        outputTimingUnit: "percent" as const,
      };
    ...
    it("accepts configuration with custom profile", () => {
      const customProfile: FormProfile = {
        name: "custom-profile",
  ...
  describe("invalid profile", () => {
    it("rejects empty profile name", () => {
      ...
      if (!result.success) {
        expect(
          result.error.issues.some((i) => i.message.includes("empty")),
        ).toBe(true);
      }
    });
  });
  ...
  describe("invalid minConfidenceThreshold", () => {
    it("rejects minConfidenceThreshold below 0", () => {
      ...
      if (!result.success) {
        expect(
          result.error.issues.some((i) => i.message.includes("at least 0")),
        ).toBe(true);
      }
    });
    ...
    it("rejects minConfidenceThreshold above 1", () => {
      ...
      if (!result.success) {
        expect(
          result.error.issues.some((i) => i.message.includes("at most 1")),
        ).toBe(true);
      }
    });
...
describe("formProfileSchema", () => {
  ...
  it("rejects empty profile name", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error.issues.some((i) => i.message.includes("empty"))).toBe(
        true,
      );
    }
  });
...
describe("createConfig", () => {
  it("returns default config when called with no arguments", () => {
    ...
    expect(config.minConfidenceThreshold).toBe(
