...
import type { FrameProvider, FrameMetadata } from "./types";
...
describe("VideoFileProvider", () => {
  describe("constructor and error cases", () => {
    ...
    describe("VideoFileCorruptedError", () => {
      it("extends Error", () => {
        const error = new VideoFileCorruptedError(
          "/path/to/video.mp4",
          "Invalid header",
        );
        expect(error).toBeInstanceOf(Error);
    ...
    describe("UnsupportedVideoFormatError", () => {
      it("extends Error", () => {
        const error = new UnsupportedVideoFormatError(
          "/path/to/video.avi",
          "avi",
        );
        expect(error).toBeInstanceOf(Error);
  ...
  describe("getMetadata()", () => {
    it("returns FrameMetadata with correct structure", async () => {
      ...
      const mockMetadata: FrameMetadata = {
        width: 1920,
        height: 1080,
