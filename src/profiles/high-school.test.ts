...
describe("highSchoolProfile", () => {
  ...
  describe("profile metadata", () => {
    ...
    it("includes age range or level in description", () => {
      expect(highSchoolProfile.description.toLowerCase()).toMatch(
        /13.*18|high.*school|intermediate|advanced.*beginner/,
      );
    });
  });
  ...
  describe("timing and rhythm metrics (new for high school)", () => {
    ...
    it("includes timing-related targets", () => {
      ...
      const hasTimingMetric = timingMetrics.some(
        (metric) => highSchoolProfile.targets[metric] !== undefined,
      );
