...
} from "./pose-shot-detector";
import type { PoseData, Frame, TestLandmark } from "../testing/types";
 ...
 */
function createStandingLandmarks(): TestLandmark[] {
 ...
 */
function createShootingLandmarks(): TestLandmark[] {
 ...
 */
function createShotSequence(): Frame[] {
  const frames: Frame[] = [];
...
describe("detectShots", () => {
  ...
  it("returns orientation with detection result", () => {
    ...
    expect(["front", "side-left", "side-right", "front-left", "front-right", "unknown"]).toContain(
      result.orientation
    );
  });
