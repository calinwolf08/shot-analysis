...
/**
 * Frame configuration for creating shot sequences.
 */
interface FrameConfig {
  // Wrist positions [leftY, rightY]
  wristY: [number, number];
  // Hip Y position (for load detection)
  hipY?: number;
  // Knee angle indicator (180 = straight, less = bent)
  kneeAngle?: number;
  // Index finger positions (for hand separation)
...
describe("PhaseDetector", () => {
  describe("createPhaseDetector factory", () => {
    ...
    it("creates a detector with custom config", () => {
      const config: PhaseDetectorConfig = {
  ...
  describe("gather phase detection", () => {
    ...
    it("marks gather as missing when shot starts mid-motion", () => {
      ...
      expect(
        result.phases[ShotPhase.Gather] === undefined ||
    ...
    it("follow-through is the last phase", () => {
      ...
      for (const phase of Object.values(ShotPhase)) {
        ...
        if (phaseRange && phase !== ShotPhase.FollowThrough) {
          expect(followThrough!.startFrame).toBeGreaterThanOrEqual(
            phaseRange.startFrame,
          );
        }
      }
    });
  ...
  describe("phase continuity and hysteresis", () => {
    it("phases do not flicker with small movements", () => {
      ...
      for (const phase of Object.values(ShotPhase)) {
        const phaseRange = result.phases[phase];
        if (phaseRange) {
          expect(phaseRange.endFrame).toBeGreaterThanOrEqual(
            phaseRange.startFrame,
          );
        }
      }
  ...
  describe("edge cases", () => {
    it("handles partial shot missing early phases", () => {
      ...
      expect(
        result.phases[ShotPhase.SetPoint] !== undefined ||
          result.phases[ShotPhase.Release] !== undefined ||
          result.phases[ShotPhase.FollowThrough] !== undefined,
      ).toBe(true);
    });
