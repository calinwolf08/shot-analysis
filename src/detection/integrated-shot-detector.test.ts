...
describe("ShotDetector", () => {
  describe("constructor and initialization", () => {
    ...
    it("creates a detector with custom config", () => {
      const config: ShotDetectorConfig = {
        boundaryConfig: {
  ...
  describe("processFrames() batch processing", () => {
    ...
    it("includes frame range in detected shots", () => {
      ...
      expect(shots[0]!.frameRange.end).toBeGreaterThan(
        shots[0]!.frameRange.start,
      );
    });
  ...
  describe("getDetectedShots()", () => {
    ...
    it("returns empty array before any shots completed", () => {
      const detector = createShotDetector();
      const stationaryPositions: Array<[number, number]> = Array(10).fill([
        0.65, 0.65,
      ]) as Array<[number, number]>;
      const sequence = createFrameSequenceWithWrists(stationaryPositions);
  ...
  describe("edge cases", () => {
    it("handles very long sequences without memory issues", () => {
      ...
      const longNeutral: Array<[number, number]> = Array(300).fill([
        0.65, 0.65,
      ]) as Array<[number, number]>;
      const shot = createCompleteShotSequence();
      const moreNeutral: Array<[number, number]> = Array(300).fill([
        0.65, 0.65,
      ]) as Array<[number, number]>;
    ...
    it("assigns phases within shot frame range", () => {
      ...
      if (shots.length > 0) {
        ...
        for (const phase of Object.values(ShotPhase)) {
          ...
          if (phaseRange) {
            expect(phaseRange.startFrame).toBeGreaterThanOrEqual(
              shot.frameRange.start,
            );
            expect(phaseRange.endFrame).toBeLessThanOrEqual(
              shot.frameRange.end,
            );
          }
        }
