...
import {
  ...
  type ShotBoundaryDetectorConfig,
 ...
 */
function createFrameSequenceWithWrists(
  wristPositions: Array<[number, number]>,
): PoseLandmarks[] {
...
function createCompleteShotSequence(): PoseLandmarks[] {
  ...
  const wristPositions: Array<[number, number]> = [
...
describe("ShotBoundaryDetector", () => {
  describe("createShotBoundaryDetector factory", () => {
    ...
    it("creates a detector with custom config", () => {
      const config: ShotBoundaryDetectorConfig = {
  ...
  describe("shot start detection", () => {
    ...
    it("does not detect shot start for stationary hands", () => {
      ...
      const stationaryPositions: Array<[number, number]> = Array(30).fill([
        0.65, 0.65,
    ...
    it("does not detect shot start for downward hand movement", () => {
      ...
      const downwardPositions: Array<[number, number]> = [];
      for (let i = 0; i < 30; i++) {
    ...
    it("ignores pump fakes (brief upward motion that reverses)", () => {
      ...
      const pumpFakePositions: Array<[number, number]> = [
        // Neutral
        ...(Array(5).fill([0.65, 0.65]) as Array<[number, number]>),
        ...
        [0.65, 0.65],
        // Continue neutral
        ...(Array(10).fill([0.65, 0.65]) as Array<[number, number]>),
      ];
    ...
    it("handles noisy landmark data with smoothing", () => {
      ...
      const shotStarts = boundaries.filter(
        (b: DetectedBoundary) => b.type === "start",
      );
      expect(shotStarts.length).toBeGreaterThanOrEqual(1);
    });
  });
  ...
  describe("shot end detection", () => {
    it("detects shot end when arm returns to neutral position", () => {
      ...
      const shotEnds = boundaries.filter(
        (b: DetectedBoundary) => b.type === "end",
      );
    ...
    it("detects shot end frame after the shot start", () => {
      ...
      expect(shotEnds[0]!.frameIndex).toBeGreaterThan(
        shotStarts[0]!.frameIndex,
      );
    });
  ...
  describe("multiple shots detection", () => {
    it("detects multiple shots in a sequence", () => {
      ...
      const gap: Array<[number, number]> = Array(20).fill([
        0.65, 0.65,
      ]) as Array<[number, number]>;
  ...
  describe("partial shot handling", () => {
    describe("video starts mid-shot", () => {
      it("marks shot as partial start at frame 0 when video begins mid-shot", () => {
        ...
        const midShotStart: Array<[number, number]> = [
      ...
      it("detects shot start at frame 0 for video starting mid-rise", () => {
        ...
        const midRise: Array<[number, number]> = [
        ...
        const starts = boundaries.filter(
          (b: DetectedBoundary) => b.type === "start",
        );
        const ends = boundaries.filter(
          (b: DetectedBoundary) => b.type === "end",
        );
    ...
    describe("video ends mid-shot", () => {
      it("marks shot as partial end at last frame when video ends mid-shot", () => {
        ...
        const midShotEnd: Array<[number, number]> = [
          // Neutral
          [0.65, 0.65],
          [0.65, 0.65],
          [0.65, 0.65],
    ...
    describe("video both starts and ends mid-shot", () => {
      it("handles video that captures only middle of a shot", () => {
        ...
        const midToMid: Array<[number, number]> = [
  ...
  describe("edge cases", () => {
    ...
    it("handles very fast shots (minimal frames)", () => {
      ...
      const fastShot: Array<[number, number]> = [
    ...
    it("does not crash on inconsistent landmark data", () => {
      ...
      const inconsistentPositions: Array<[number, number]> = [
        [0.65, 0.65],
        [0.5, 0.5],
