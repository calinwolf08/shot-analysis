...
import type { Shot, ShotPhases } from "./types";
import {
...
/**
 * Internal state for tracking shot progress during incremental processing.
 */
interface ShotState {
  /** Currently in a shot motion */
  inShot: boolean;

  /** Frame index where current shot started */
  shotStartFrame: number;

  /** Index of current shot being detected */
  currentShotIndex: number;

  /** Completed shots */
  completedShots: Shot[];

  /** All accumulated frames */
  accumulatedFrames: PoseLandmarks[];
...
export class ShotDetector {
   ...
   */
  private createInitialState(): ShotState {
    return {
      inShot: false,
      shotStartFrame: -1,
      currentShotIndex: 0,
 ...
 */
export function createShotDetector(
